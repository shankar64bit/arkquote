...
class EditEfElrPage extends StatelessWidget with GetItMixin {
  Map args = {};
  EditEfElrPage(@required this.args, {Key key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final EfElrModel _efelrModel = watchOnly((EfElrProvider x) => x.efelrModel);
    _etypEcontroller = _efelrModel.etype;
    _IDcontroller.text = args['id'].toString();
    _trNo_Controller.text = _efelrModel.trNo.toString();
    _LocatioNcontroller.text = _efelrModel.location;
    _serialNo_Controller.text = _efelrModel.serialNo;
    _make_Controller.text = _efelrModel.make;
    _rtype_Controller.text = _efelrModel.rtype;
    _panel_Controller.text = _efelrModel.panel;
    _auxVoltage_Controller.text = _efelrModel.auxVoltage.toString();
    _ctRatio_Controller.text = _efelrModel.ctRatio;
    _DOTcontroller.text =
        DateFormat('dd/MM/yyyy').format(_efelrModel.dateOfTesting).toString();
    designation = _efelrModel.designation;
    _dateOfTesting = _efelrModel.dateOfTesting;
    _witnessedBy_Controller.text = _efelrModel.WitnessedBy;

    var mob = const Text('Edit Earth Fault /Leakage Relay Details',
        style: TextStyle(fontSize: 15));
    var desk = const Text('Edit Earth Fault / Leakage Relay Details',
          ...
          IconButton(
              onPressed: () async {
                if (!_formKey.currentState.validate()) {
                  ...
                  var url = Uri.parse(
                      '${Env.URL_PREFIX}/efelRelay/${ntr.databaseID}/');
                  ...
                  } else if (_etypEcontroller == 'efrr') {
                    designation = 'Restricated Earth Fault Relay';
                  } else if (_etypEcontroller == 'efrs') {
                  ...
                  if (response.statusCode == success_code ||
                    ...
                    final EfElrModel efelrReportToLocal = EfElrModel(
                      ...
                      WitnessedBy: _witnessedBy_Controller.text.toString(),
                      etype: _etypEcontroller,
                      databaseID: _efelrModel.databaseID,
                      updateDate: DateTime.now(),
                    );
                    getX((EfElrProvider x) =>
                        x.updateEfElr(efelrReportToLocal, args['id']));
