...
class AddBbHvPage extends StatelessWidget with GetItMixin {
  Map args = {};
  int TR_ID;
  int BB_ID;

  AddBbHvPage(this.args, {Key key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    var mob =
        const Text('Add Busbar-HV Details', style: TextStyle(fontSize: 15));
    var desk =
        const Text('Add Busbar-HV Details', style: TextStyle(fontSize: 20));
          ...
          IconButton(
              onPressed: () async {
                if (!_formKey.currentState.validate()) {
                  ...
                  getX((BbHvProvider x) => x.addBbHv(bbHvReportToLocal));
                  watchOnly((BbHvProvider x) => {});
                  print("Busbar Hv added");
            ...
            child: Card(
              ...
              child: Column(children: [
                ...
                simplifiedFor_Diplay_for_all_PC_numpad(
                    'R-Phase Measured Leakage Current (mA)', _Rcontroller),
                simplifiedFor_Diplay_for_all_PC_numpad(
                    'Y-Phase Measured Leakage Current (mA)', _Ycontroller),
                simplifiedFor_Diplay_for_all_PC_numpad(
                    'B-Phase Measured Leakage Current (mA)', _Bcontroller),
                ntr.noOfBar == 4
                    ? simplifiedFor_Diplay_for_all_PC_numpad(
                        'Neutral Measured Leakage Current (mA)', _Ncontroller)
                    : Container(),
              ]),
            ),
          );
