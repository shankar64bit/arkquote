...
class EditEETestPage extends StatelessWidget with GetItMixin {
  Map args = {};

  EditEETestPage(this.args, {Key key}) : super(key: key);
  int _locRef;
  ...
  Future<void> getReport() async {
    getX((EETestProvider x) => x.getEETestByID(args['id']));
    _eeTestModel = watchOnly((EETestProvider x) => x.eeTestModel);
    _locNOController.text = args['EE_LOC_ID'].toString();

    _IDcontroller.text = args['id'].toString();
    _nOcontroller.text = args['trNo'].toString();
    _eeNamecontroller.text = _eeTestModel.eeName;
    _resistValuecontroller.text = _eeTestModel.resistanceValue.toString();
    _locRef = _eeTestModel.locNo;
  }

  @override
  Widget build(BuildContext context) {
    double resistanceValue;
    final EETestModel ntr = watchOnly((EETestProvider x) => x.eeTestModel);
    getReport();
    var mob =
        const Text('Edit EE-Test Details', style: TextStyle(fontSize: 15));
    var desk =
        const Text('Edit EE-Test Details', style: TextStyle(fontSize: 20));
              ...
              onPressed: () async {
                if (!_formKey.currentState.validate()) {
                  ...
                  if (response.statusCode == success_code ||
                    ...
                    getX((EETestProvider x) =>
                        x.updateEETest(eeTestReportToLocal, args['id']));
              ...
              Expanded(
                child: AlertDialog(
                  ...
                  actions: [
                    TextButton(
                      onPressed: () {
                        getX((EETestProvider x) => x.deleteEETest(args['id']));
                        Navigator.pop(context);
                        Navigator.pop(context);
                      },
                      child: const Text(
