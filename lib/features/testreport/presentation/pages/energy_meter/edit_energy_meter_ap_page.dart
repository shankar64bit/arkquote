...
class EditEnergyMeterApPage extends StatelessWidget with GetItMixin {
  Map args = {};

  EditEnergyMeterApPage(this.args, {Key key}) : super(key: key);

  EnergyMeterApTestModel _energymeterApTestModel;
  ...
  Future<void> getReport() async {
    ...
    _itRcontroller.text =
        _energymeterApTestModel.initialTestMeterReading.toString();
    _atRcontroller.text =
        _energymeterApTestModel.afterTestMeterReading.toString();
    _tr_Rcontroller.text =
        _energymeterApTestModel.testMeterReading_R.toString();
    _isRcontroller.text =
        _energymeterApTestModel.initialStandardMeterReading.toString();
    _asRcontroller.text =
        _energymeterApTestModel.afterStandardMeterReading.toString();
    _SerialNOcontroller.text = args['Serial_No'];
    _sr_Acontroller.text =
        _energymeterApTestModel.standardMeterReading_A.toString();
  }

  @override
  Widget build(BuildContext context) {
    final EnergyMeterApTestModel ntr =
        watchOnly((EnergyMeterApProvider x) => x.energymeterApModel);
    getReport();
    var mob = const Text('Edit Energy Meter-AP Details',
        style: TextStyle(fontSize: 15));
    var desk = const Text('Edit Energy Meter-Ap Details',
          ...
          IconButton(
              onPressed: () async {
                if (!_formKey.currentState.validate()) {
                  ...
                  var url = Uri.parse('${Env.URL_PREFIX}/apTest/' +
                  ...
                  if (response.statusCode == success_code ||
                    ...
                    final EnergyMeterApTestModel energyMeterApReportToLocal =
                    ...
                    getX((EnergyMeterApProvider x) => x.updateEnergyMeterAp(
                        energyMeterApReportToLocal, args['id']));
                    Navigator.pop(context);

                    Navigator.popAndPushNamed(context, '/detail_EM_AP',
                        arguments: {
                          'id': args['id'],
                          'EM_ID': args['EM_ID'],
