...
class EditEnergyMeterFiPage extends StatelessWidget with GetItMixin {
  Map args = {};

  EditEnergyMeterFiPage(this.args, {Key key}) : super(key: key);

  EnergyMeterFiTestModel _energymeterFiTestModel;
  ...
  Future<void> getReport() async {
    ...
    _energymeterFiTestModel =
        watchOnly((EnergyMeterFiProvider x) => x.energymeterFiModel);

    _IDcontroller.text = args['id'].toString();
    _nOcontroller.text = args['TrNo'].toString();

    _brcontroller.text = _energymeterFiTestModel.br.toString();
    _rrcontroller.text = _energymeterFiTestModel.rr.toString();
    _yrcontroller.text = _energymeterFiTestModel.yr.toString();
    _yacontroller.text = _energymeterFiTestModel.ya.toString();
    _bacontroller.text = _energymeterFiTestModel.ba.toString();
    _SerialNOcontroller.text = args['Serial_No'];
    _racontroller.text = _energymeterFiTestModel.ra.toString();
  }

  @override
  Widget build(BuildContext context) {
    final EnergyMeterFiTestModel ntr =
        watchOnly((EnergyMeterFiProvider x) => x.energymeterFiModel);
    getReport();
    var mob = const Text('Edit Energy Meter- FI Details',
        style: TextStyle(fontSize: 15));
    var desk = const Text('Edit Energy Meter -FI Details',
          ...
          IconButton(
              onPressed: () async {
                if (!_formKey.currentState.validate()) {
                  ...
                  //add to remote database
                  var url = Uri.parse('${Env.URL_PREFIX}/freqiTest/' +
                  ...
                  if (response.statusCode == success_code ||
                    ...
                    final EnergyMeterFiTestModel energyMeterFiReportToLocal =
                    ...
                    getX((EnergyMeterFiProvider x) => x.updateEnergyMeterFi(
                        energyMeterFiReportToLocal, args['id']));
                    Navigator.pop(context);

                    Navigator.popAndPushNamed(context, '/detail_EM_FI',
                        arguments: {
                          'id': args['id'],
                          'EM_ID': args['EM_ID'],
