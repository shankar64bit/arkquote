...
class EditEnergyMeterViPage extends StatelessWidget with GetItMixin {
  Map args = {};

  EditEnergyMeterViPage(this.args, {Key key}) : super(key: key);

  EnergyMeterViTestModel _energymeterViTestModel;
  ...
  Future<void> getReport() async {
    ...
    _energymeterViTestModel =
        watchOnly((EnergyMeterViProvider x) => x.energymeterViModel);

    _IDcontroller.text = args['id'].toString();
    _nOcontroller.text = args['TrNo'].toString();

    _brcontroller.text = _energymeterViTestModel.br.toString();
    _rrcontroller.text = _energymeterViTestModel.rr.toString();
    _yrcontroller.text = _energymeterViTestModel.yr.toString();
    _yacontroller.text = _energymeterViTestModel.ya.toString();
    _bacontroller.text = _energymeterViTestModel.ba.toString();
    _SerialNOcontroller.text = args['Serial_No'];
    _racontroller.text = _energymeterViTestModel.ra.toString();
  }

  @override
  Widget build(BuildContext context) {
    final EnergyMeterViTestModel ntr =
        watchOnly((EnergyMeterViProvider x) => x.energymeterViModel);
    getReport();
    var mob = const Text('Edit Energy Meter-VI Details',
        style: TextStyle(fontSize: 15));
    var desk = const Text('Edit Energy Meter-VI Details',
          ...
          IconButton(
              onPressed: () async {
                if (!_formKey.currentState.validate()) {
                  ...
                  //add to remote database
                  var url = Uri.parse('${Env.URL_PREFIX}/viTest/' +
                  ...
                  if (response.statusCode == success_code ||
                    ...
                    getX((EnergyMeterViProvider x) => x.updateEnergyMeterVi(
                        energyMeterViReportToLocal, args['id']));
                    Navigator.pop(context);

                    Navigator.popAndPushNamed(context, '/detail_EM_VI',
                        arguments: {
                          'id': args['id'],
                          'EM_ID': args['EM_ID'],
