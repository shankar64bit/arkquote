...
import 'package:flutter_testreports/features/testreport/domain/entities/equipment/isolator/iso_ir.dart';
...
class DetailIsoIrPage extends StatelessWidget with GetItMixin {
  Map args = {};
  int ISO_ID;

  DetailIsoIrPage(this.args, {Key key}) : super(key: key);

  IsoIrTestModel isoIrTestModel, ntr;

  @override
  Widget build(BuildContext context) {
    final IsoModel ntr1 = watchOnly((IsoProvider x) => x.isoModel);
    getX((IsoIrProvider x) => x.getIsoIrByID(args['id']));
    IsoIrTestModel ntr = watchOnly((IsoIrProvider x) => x.isoIrModel);

    print(ntr);
    print("ntr  ......");
    var mob =
        const Text('Iso - Ir Test Details', style: TextStyle(fontSize: 15));
    var desk =
        const Text('ISo - Ir Test Details', style: TextStyle(fontSize: 20));
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
              onPressed: () {
                Navigator.popAndPushNamed(context, '/edit_ISO_IR', arguments: {
                  'id': args['id'],
                  'TrNo': ntr.trNo,
                  'ISO_ID': args['ISO_ID'],
          ...
          IconButton(
            onPressed: () {
              getX((IsoIrProvider x) => x.deleteIsoIr(args['id']));
                      ...
                      child: Column(
                        ...
                        children: [
                          ConstrainedBox(
                              ...
                              child: display_ISO_IR_details(ntr)),
                        ],
                      ),
                    ),
                  ]),
  ...
  Container display_ISO_IR_details(IsoIrTest ntr) {
    return Container(
      child: Column(
        children: [
          Column(
                  ...
                  children: [
                    ...
                    Card(
                      ...
                      child: Padding(
                        padding: const EdgeInsets.all(5.0),
                        child: Column(children: [
                          Text('re : ' + ntr.re.toString(),
                              style: const TextStyle(
                                  color: Colors.black, fontSize: 13)),
                          Divider(height: 10),
                          Text('ye : ' + ntr.ye.toString(),
                              style: const TextStyle(
                                  color: Colors.black, fontSize: 13)),
                          Divider(height: 10),
                          Text('be : ' + ntr.be.toString(),
                              style: const TextStyle(
                                  color: Colors.black, fontSize: 13)),
                        ]),
                      ),
                    ...
                    Card(
                      ...
                      child: Padding(
                        padding: const EdgeInsets.all(5.0),
                        child: Column(children: [
                          Text('ry : ' + ntr.ry.toString(),
                              style: const TextStyle(
                                  color: Colors.black, fontSize: 13)),
                          Divider(height: 10),
                          Text('yb : ' + ntr.yb.toString(),
                              style: const TextStyle(
                                  color: Colors.black, fontSize: 13)),
                          Divider(height: 10),
